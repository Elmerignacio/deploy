...
<x-trea-components.sidebar :profile="$profile"  :firstname="$firstname" :lastname="$lastname">
    ...
    <div class="mt-6 pb-5">
        ...
        <thead class="bg-[#1a4d2e] border border-black text-white">
        ...
        <tbody id="usersTableBody">
            ...
            <tr class="border border-black"
                ...
                <td class="p-2 border border-black text-center">{{ $payable->description }}</td>
                <td class="p-2 border border-black text-right">₱{{ number_format(floor($payable->input_balance), 2) }}</td>
                <td class="p-2 border border-black text-right">₱{{ number_format(floor($payable->expected_receivable), 2) }}</td>
                <td class="p-2 border border-black text-center">{{ \Carbon\Carbon::parse($payable->dueDate)->format('F d, Y') }}</td>
            </tr>
            @endforeach
        </tbody>
       </x-scrollable-table>

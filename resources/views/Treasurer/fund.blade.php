            ...
            <div x-data="remittanceApp({{ $remittanceRecords->toJson() }})" class="flex flex-col md:flex-row">
                ...
                <div class="w-full">
                    ...
                    <div id="printArea">
                        <table class="mt-4 w-full table-fixed border border-black">
                            <thead>
                                <tr class="bg-white text-center">
                                    ...
                                    <th class="p-2 border border-black bg-green-700 text-white">EXPENSES</th>
                                    <th class="p-2 border border-black text-black">CASH ON HAND</th>
                                </tr>
                            </thead>
                        </table>
                                ...
                                <tbody>
                                    @foreach($expensesWithDescriptions as $expense)
                                        <tr class="text-center">
                                            ...
                                            <td class="p-2 border border-black text-black">{{ number_format($totalExpenses, 2) }}</td>
                                            <td class="p-2 border border-black text-black">{{ number_format($cashOnHand, 2) }}</td>
                        ...
                        <table class="w-full table-fixed border border-black">
                            <tfoot>
                                <tr class="bg-green-700 text-center font-semibold">
                                    <td class="p-2 border border-black text-white">TOTAL</td>
                                    <td class="p-2 border border-black text-white">{{ number_format($totalExpenses, 2) }}</td>
                                    <td class="p-2 border border-black text-white">{{ number_format($cashOnHand, 2) }}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                    <div class="flex justify-end gap-4 mt-6 no-print">
                        <button @click="printSummary" class="bg-green-700 text-white px-4 py-2 rounded hover:bg-gray-200 font-bold">🖨 PRINT</button>
                        <button @click="exportSummary" class="bg-green-700 text-white px-4 py-2 rounded hover:bg-gray-200 font-bold">📁 EXPORT</button>
                    </div>
                </div>
...
<script>
    ...
    printWindow.document.write('<html><head><title>Expenses Summary</title>');
    ...
    printWindow.document.write('<h3 style="text-align: center; font-family: Arial, sans-serif;">Expenses Summary</h3>');

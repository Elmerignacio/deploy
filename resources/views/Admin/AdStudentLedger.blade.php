...
<x-trea-components.content>
  <x-Add-components.sidebar :profile="$profile"  :firstname="$firstname" :lastname="$lastname">
    ...
    <div class="mt-4 text-[#1a4d2e]">
        ...
        <p class="text-gray-700 text-2xl">ID: {{ $student->IDNumber }}</p>
        <p class="text-gray-700 text-2xl">{{ strtoupper($student->yearLevel) }} - {{ strtoupper($student->block) }}</p>
    </div>
    
  <div class="flex flex-col md:flex-row gap-6 mt-6">

<div class="md:w-1/3">
    <h3 class="text-lg font-bold text-red-700">REMAINING BALANCE:</h3>
            ...
            <tbody>
                @foreach($payables as $payable)
                    <tr class="bg-white">
                        ...
                        <td class="p-3 border border-black font-bold text-right">₱{{ number_format($payable->total_balance, 2) }}</td>
                    </tr>
                @endforeach
                <tr class="bg-[#1a4d2e] text-white font-bold">
                    <td class="p-3 border border-black text-center">TOTAL</td>
                    <td class="p-3 border border-black text-right">₱{{ number_format($payables->sum('total_balance'), 2) }}</td>
                </tr>
            </tbody>
        </table>
    </div>
    ...
    <div class="md:w-2/3 ">
      <h3 class="text-lg font-bold text-green-700 mb-2">SETTLED PAYABLE:</h3>
      <div class="max-h-96 overflow-y-auto">
        <table class="w-full table-auto border-separate border-spacing-0">
          ...
          <tfoot class="sticky bottom-0 bg-[#1a4d2e] text-white z-10">
            <tr>
              <td class="p-3 border border-black text-left font-bold" colspan="2">TOTAL</td>
